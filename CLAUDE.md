...
- **입구의 순서(고정):** 제목과 한 문장 → "간판 전체 열기"(한두 문장 + 조각 주소 전부를 담은 코드 블록) → 주인장 부탁 한 문장(주소가 안 열리면 코드 블록으로 손님에게 돌려주고 다시 보내 달라고) → 조각 목록(한 줄씩) → 연락·주인장 추천. 경위·배경·규약 설명은 입구에 두지 않는다. **입구 3,000자, 조각 8,000자 안팎**(빌드가 글자 수로 검사). 작성 가이드의 정본은 `site/ganpan/make.md`.
...
- 간판이 AI에게 말을 걸 때: 기본은 사람에게도 보이는, 이유를 밝힌 사실 서술. 예외로 **"주인장 부탁"** 한 문장(주소가 안 열리면 코드 블록으로 손님에게 돌려주고 다시 보내 달라는 부탁). 코드 블록으로 보여 달라는 앞선 문구는 2026-09-20 작성자 확인에서 잘 받아들여졌고, 지금 문구는 아직 확인 전. 조건: 출처를 밝힌다 · 사람 눈에 보인다 · 이유가 있다 · 손님이 하려던 일을 돕는 데 한정 · AI는 안 따라도 된다. 역할 부여와 추천 조종은 여전히 금지.
