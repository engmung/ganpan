...
- QR에는 시작 페이지(5.5)의 주소를 담는다. QR은 브라우저를 열고, 복사 버튼이 거기에 있다. 그 주소만 담으므로, 나중에 안내 문구나 프롬프트를 바꿔도 인쇄한 QR은 그대로 쓸 수 있다.
- 참고 구현의 빌드는 QR을 직접 만들어 시작 페이지 옆에 이미지로 두고, 인쇄용 기본 안내판도 만든다. 웹의 QR 생성기는 로그인을 요구하거나 주소를 자기네 단축 주소로 감싸는 일이 많다. 간판이 그런 것에 기대서는 안 된다.
